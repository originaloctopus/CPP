...
bool isPersonAllowed (std::string person)
{
	...
	for (int x = 0; x < (sizeof (ALLOW_ACCESS) / sizeof (char*)); x++)
	{
		...
		{
			is_allowed = true; //removed relational operator: == to =
			break;
		}
	}
...
void ssDoorLockEventHandler (DoorLock::DoorLockEvent event, string pin_code)//changed int to string for pin_code. May not be right!
{
...
void ssVideoDoorbellEventHandler (VideoDoorbell::VideoDoorbellEvent event, std::string person_at_door)
{
	...
	switch (event)
	{
		...
		case VideoDoorbell::VideoDoorbellEvent::SOMEONE_AT_THE_DOOR:
		{
			...
			if (isPersonAllowed (person_at_door))// ! removed before isPersonAllowed
			{
				access_granted = true;
				DoorLock::unlockDoor ();
			}
